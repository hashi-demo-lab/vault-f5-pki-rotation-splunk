...
check "certificate" {
  assert {
    ...
    error_message = <<-EOF
    Certificate Renewal Pending: ${vault_pki_secret_backend_cert.this.common_name}<br>
    min_seconds_remaining: ${var.min_seconds_remaining}<br>
    auto_renew: ${var.auto_renew}<br>
    EOF
  }
}
